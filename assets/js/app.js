...
//////////////////////////////////////////////////////////////
$('#backComment').on('click',function(e){
  window.location.href = document.referrer;
})

$('#backScanButton').on('click',function(e){
    window.location.href = document.referrer;
})
...
if($('#candidate_profile').length > 0){
  ...
  $('#comment_loading').bind('input propertychange',function(e){
    $.ajax({
      type: "POST",
  ...
  $("#cancelSearchCandidat").on("click", function(){

  })
  //handle favorites
  $(".filterComment>.fa").on("click", function(){
  ...
  if($("#readerScan").length){
    $( "#html5-qrcode-button-camera-permission" ).on( "load", function() {
      console.log('oui boutton');
      $("#html5-qrcode-button-camera-permission").trigger('click');
      // Handler for `load` called.
    } )
    if($(".html5-qrcode-element").length){
      console.log('oui boutton');
      $(".html5-qrcode-element").trigger('click');
    }
    function onScanSuccess(decodedText, decodedResult) {
      ...
      var data = JSON.parse(atob(decodedText));
      $.ajax({
        type : 'POST',
        ...
        success: function(response){
          alert('validé');
          window.location.href="/scan/today";
        }
      });
    }
  ...
  if($("#reader").length){
    function onScanSuccess(decodedText, decodedResult) {
      // handle the scanned code as you like, for example:
      var data = JSON.parse(atob(decodedText));
      var id = $("#reader").attr("participationId");
      $.ajax({
        type : 'POST',
        data: JSON.stringify(data),
        url:"/admin-exposant/addScan/"+id,
        cache: false,
        success: function (data) {
          console.log(data);
          if(data.error){
            $('.errorContainer').remove();
            $("#scan_candidate").before( "<div class=\"errorContainer\"><p>"+data.error+"</p></div>" );
          }else{
            window.location.href="/admin-exposant/voir-candidat/"+data;
          }
        }
      });
    }
  ...
  $('#searchCandidatScan').on("click", function() {
        $.ajax({
          type: "POST",
          url: "/admin-exposant/searchScan",
          data: {
            info: $("#infoCandidatScan").val(),
            id: $("#idParticipation").val()
          },
          cache: false,
          success: function (data) {
              if(data.error){
  ...
  $('#searchCandidatScanScan').on("click", function() {
        $.ajax({
          type: "POST",
          url: "/scan/searchScan",
          data: {
          ...
          success: function (data) {
            if(data.error){
              ...
              alert("Le candidat a bien été scanné")
            }
          }
        })
      }
