...
export default function UniqueNextEvent (props) {
                ...
                <a className={"eventButton"} href={"/"+data.slug}>
                    ...
                    { data.childEvents.length > 0 ?
                        <p style={{textAlign:"center"}} className={"textEventBot"}>
                            <span className={"date"} style={styles.coloredText}>Du {moment(data.date).format('dddd DD MMMM')} au {moment(data.childEvents[0].date).format('dddd DD MMMM')}</span>
                            <br/>
                        ...
                        :
                        <p style={{textAlign:"center"}} className={"textEventBot"}>
                            <span className={"date"} style={styles.coloredText}>{moment(data.date).format('dddd DD MMMM')}</span>
                            <br/>
                            <span className={"heure"} style={styles.whiteText}>{moment(data.date, 'yyyy-MM-dd’T’hh:mm:ss.SSSZ').format('HH')}h - {moment(data.closingAt, 'yyyy-MM-dd’T’hh:mm:ss.SSSZ').format('HH')}h{moment(data.closingAt, 'yyyy-MM-dd’T’hh:mm:ss.SSSZ').format('mm') !== "00" && moment(data.closingAt, 'yyyy-MM-dd’T’hh:mm:ss.SSSZ').format('mm')}</span>
                        </p>
                    }
